/* 
 ...
 1. User will set the time between two successive readings.
 2. Program reads the values set on POTs for time unit and time count
 and will find the time for each reading.
 3. If start key is pressed, it will read the analog input at pin A0.
 4. This digital count is divided by 4 and it will store the value to EEPROM
 delay of the user specified count is given and it will repeat the procedure untill all the 1023 EEPROM addresses are filled.
 5. After this the controller will turn the LED at pin D13. 
 6. To read the data user has to press READ key connected to pin D7.
 7. If READ key is pressed , program will send the values stored in EEPROM to serial monitor.
 (data collected on serial terminal is in CSV form)
